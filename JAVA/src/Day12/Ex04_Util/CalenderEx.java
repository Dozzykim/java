...
public class CalenderEx {
	...
	public static void printCalender(String msg, Calendar cal) {
		...
		int month = cal.get( Calendar.MONTH ) + 1;			// 月월(0~11)  !!주의: 0이 1월을 의미하므로, 1월이면 0으로 지정해야함. or 상수값으로 지정하던지.
	...
	public static void main(String[] args) {
		//Calendar는 싱글톤 패턴의 단일객채로, new Calendar(); 대신 getInstance()메소드로 객체를 사용함.
		...
		printCalender("자바 마지막 수업일(연~초)", test);
		
		//종강일
		Calendar lastDay = Calendar.getInstance();
		lastDay.set(2024, Calendar.AUGUST, 1, 18, 0 , 0);
		printCalender("종강일", lastDay);
		...
		System.out.println("1970년 1월 1일 0시 0분 0초 0ms ~ 오늘까지 일 수 : " + nowD + "일");
		System.out.println("1970년 1월 1일 0시 0분 0초 0ms ~ 종강까지 일 수 : " + lastD + "일");
		
		//내가 태어난 지 며칠~?
		Calendar myBday = Calendar.getInstance();
		myBday.set(1997, Calendar.AUGUST, 24, 8, 30, 0);
		long myBdays = myBday.getTimeInMillis() / 1000 / 60 / 60 /24;
		long afterBirth = nowD - myBdays;
		System.out.println("생후 " + afterBirth + "일이 지났습니다.");
		
		if (now.before(lastDay)) {
			System.out.println("종강 전입니다.");
		} else if (now.after(lastDay)) {
			System.out.println("종료된 강의입니다.");
		}
