...
public class Ex07_File {
	...
	*/
	private static void makeDirectory(String filePath) {
		// 파일 객체 생성 방법
		// 1. File 객체 명 = new File(파일경로);
		// 2. File 객체 명 = new File(폴더경로, 파일명);
		File file = new File(filePath);
		
		//폴더생성
		//mkdir() 	: 생성할 경로의 상위 경로까지 기존 디렉토리가 존재해야한다.
		//mkdirs() 	: 생성할 경로의 상위 경로까지 디렉토리를 생성해준다.
		//ex) C:\A\B => 만약 A폴더가 없으면 mkdirs는 해당 폴더까지 생성해줌.
	...
	*/
	private static void listDirectory(String filePath) {
		...
		System.out.println("------------------");
		File file = new File(filePath);
		File[] fileList = file.listFiles(); //하위 경로의 파일목록을 File배열로 반환
		
		for (File subFile : fileList) {
			long update = subFile.lastModified(); //lastModified();메소드는 마지막 수정일을 가져옴.
	...
	*/
	...
	private static void fileInfo(String filePath) {
		...
		if (file.isDirectory()) {
			System.out.println("디렉토리");  //디렉토리 == 폴더
		}
		if (file.isFile()) {
			System.out.println("일반파일");
		}
		
		long update = file.lastModified(); //lastModified();메소드는 마지막 수정일을 가져옴.
		Date date = new Date(update);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		String updateDate = sdf.format(date);
